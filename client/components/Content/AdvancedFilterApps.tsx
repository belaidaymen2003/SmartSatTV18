  ...
  return (
    <div className="relative" ref={ref as any}>
      ...
      <button
        ...
        className="w-full mt-2 px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white text-left flex items-center justify-between"
      >
        <span className="truncate">{value.length ? value.join(', ') : 'Any'}</span>
        <svg className={`ml-2 w-4 h-4 text-white/60 transition-transform ${open ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor">
          <path strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>
          ...
          {options.map(opt => (
            <li key={opt.value} className="px-3 py-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={value.includes(opt.value)} onChange={() => toggle(opt.value)} className="w-4 h-4 text-blue-500" />
...
export default function AdvancedFilterApps({
  ...
  const [internet, setInternet] = useState<boolean | null>(initial.internetConnection ?? null)
  const [minStorage, setMinStorage] = useState<string>(initial.minStorage?.toString() || '')
  const [maxStorage, setMaxStorage] = useState<string>(initial.maxStorage?.toString() || '')
  const [sortBy, setSortBy] = useState<AppFilters['sortBy']>(initial.sortBy || 'newest')
  const [sortDir, setSortDir] = useState<AppFilters['sortDir']>(initial.sortDir || 'desc')
    ...
    <div className="glass rounded-xl p-6 mb-8 border border-white/10">
      <div className="flex flex-col md:flex-row gap-4 md:items-end md:justify-between">
        ...
        <div className="w-40">
          ...
          <input value={minCredit} onChange={e => setMinCredit(e.target.value.replace(/[^0-9]/g, ''))} className="w-full mt-2 px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white" placeholder="0" />
        </div>

        <div className="w-40">
          <label className="text-sm text-white/60">Max Credits</label>
          <input value={maxCredit} onChange={e => setMaxCredit(e.target.value.replace(/[^0-9]/g, ''))} className="w-full mt-2 px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white" placeholder="Any" />
        </div>

        <div className="w-48">
          <label className="text-sm text-white/60">Version</label>
          <input value={version} onChange={e => setVersion(e.target.value)} className="w-full mt-2 px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white" placeholder="e.g. 1.2.3" />
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
        <div className="flex gap-3 items-center">
          <div className="w-56">
            <MultiSelectDropdown label="Platforms" options={platformOptions} value={platforms} onChange={setPlatforms} />
          </div>
          ...
          <div className="w-44">
            <Dropdown
              ...
              onChange={(v: any) => setInternet(v === 'yes' ? true : v === 'no' ? false : null)}
              placeholder="Any"
              options={[{ value: undefined, label: 'Any' }, { value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' } as any]}
            />
          </div>

          <div className="w-40">
            <label className="text-sm text-white/60">Min Storage (MB)</label>
            <input value={minStorage} onChange={e => setMinStorage(e.target.value.replace(/[^0-9]/g, ''))} className="w-full mt-2 px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white" placeholder="0" />
          </div>

          <div className="w-40">
            <label className="text-sm text-white/60">Max Storage (MB)</label>
            <input value={maxStorage} onChange={e => setMaxStorage(e.target.value.replace(/[^0-9]/g, ''))} className="w-full mt-2 px-3 py-2 rounded-md bg-white/5 border border-white/10 text-white" placeholder="Any" />
          </div>
        </div>
