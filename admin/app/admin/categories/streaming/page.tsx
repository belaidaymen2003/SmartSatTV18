      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-3 justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="px-2 py-1 rounded border border-white/10 text-white/80 hover:bg-white/10" onClick={() => setAsc((v) => !v)} aria-label="Toggle sort order">
              {asc ? "Asc" : "Desc"}
            </button>
          </div>
                  ...
                  <tr key={s.id || s.code} className="bg-black/30 border border-white/10 rounded">
                    <td className="px-3 py-2 align-middle">
                      {editingId === s.id ? (
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => copyCode(s.code)} className="text-white hover:underline" title="Copy code">
                            {s.code}
                          </button>
                        </div>
                      )}
                    ...
                    <td className="px-3 py-2 align-middle text-white/80">
                      {editingId === s.id ? (
                        <select
                          ...
                          onChange={(e) => {
                            setEditValues((ev) => ({ ...ev, duration: Number(e.target.value) }));
                          }}
                    ...
                    <td className="px-3 py-2">
                      <span className="inline-flex items-center px-2 py-0.5 rounded border text-xs border-emerald-500/30 text-emerald-400">{s.status || "ACTIVE"}</span>
                    </td>
                    <td className="px-3 py-2">
                      <div className="flex gap-2">
                        {editingId === s.id ? (
                          <>
                            <button onClick={() => saveEdit(s.id)} className="px-2 py-1 rounded border border-green-500 text-green-400">
                              Save
                            </button>
                            <button onClick={cancelEdit} className="px-2 py-1 rounded border border-white/10">
                              Cancel
                            </button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => startEdit(s)} className="px-2 py-1 rounded border border-white/10 hover:bg-white/10 text-white/80">
                              Edit
                            </button>
                            <button onClick={() => removeSubWithAuth(s.id)} className="px-2 py-1 rounded border border-red-500/30 text-red-400 hover:bg-red-500/10">
                              Delete
                            </button>
                          </>
                        )}
        ...
        {total > perPage && (
          <div className="flex items-center justify-between text-xs text-white/60">
            <div>Page {pageSub} of {Math.max(1, Math.ceil(total / perPage))}</div>
            <div className="flex gap-2">
              <button disabled={pageSub <= 1} onClick={() => setPageSub((p) => Math.max(1, p - 1))} className="px-3 py-1 rounded border border-white/10 disabled:opacity-50">
                Prev
              </button>
              <button disabled={start + perPage >= total} onClick={() => setPageSub((p) => p + 1)} className="px-3 py-1 rounded border border-white/10 disabled:opacity-50">
                Next
              </button>
            </div>
          </div>
