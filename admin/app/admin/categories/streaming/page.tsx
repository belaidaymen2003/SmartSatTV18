...
export default function StreamingPage() {
  ...
  function SubscriptionTable({ channelId }: { channelId: number | null }) {
    ...
    const filteredSubs = useMemo(() => {
      ...
      const base = (subs || []).filter(
        (s: any) => !qq || String(s.code).toLowerCase().includes(qq)
      );
                    ...
                    <td className="px-3 py-2 align-middle">
                      {editingId === s.id ? (
                        <input
                          ...
                          onChange={(e) =>
                            setEditValues((ev: any) => ({
                              ...ev,
                              code: e.target.value,
                            }))
                          }
                          className="bg-transparent border border-white/10 rounded px-2 py-1 text-white focus:outline-none"
                        />
                      ) : (
                        <div className="flex items-center gap-2">
                          <button
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <a
                href={`/admin/categories/add/streaming/subscription/${channel.id}`}
                rel="noreferrer"
              >
  ...
  const fetchChannels = async (pageNum = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/categories/category?${params.toString()}`
      );
      const d = await res.json().catch(() => ({}));
  ...
  const openEdit = (ch: IPTVChannel) => {
    ...
    setForm({
      ...
      category: ch.category || "STREAMING",
      type: ch.type || "OTHER",
    });
  };
  ...
  const removeChannel = async (id: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/categories/category?id=${id}`, {
        method: "DELETE",
      });
      const d = await res.json().catch(() => ({}));
          ...
          onSave={async (d, file) => {
            ...
            try {
              ...
              if (file) {
                ...
                if (edit.logo) fd.append("oldLogoUrl", edit.logo);
                fileUrl = await fetch("/api/admin/categories/upload", {
                  method: "PUT",
                  body: fd,
                }).then((res) => res.json());
              }
